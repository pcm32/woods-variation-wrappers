...
for FILEPREFIX in "${identsArray[@]}"; do
	...
	then
		echo "Separate snps and indels files found, merging them in the cluster..."
	...
	then
		echo "VCF created with different version to hg19, sending liftOver run to cluster..."
	...
	cd $VEP_DIR
	echo "Sending variant effect predictor run to cluster..."
